...
def get_around_points(rect,angle,original_image):
    ...
    points = [
        ...
        (-half_width, 0),              # Left-center
        (+half_width, 0),              # Right-center
    ]
    rotated_points = []
...
class Collision_detect:
    ...
    def detect_by_color(self, rect,original_image, the_map,angle,target_color,zoom = 1):
        ...
        for point in points:
            check_x,check_y = int(point[0]/zoom),int(point[1]/zoom)
            

            if (0 <= check_x < the_map.get_width() and
                0 <= check_y < the_map.get_height()):
                pixel_color = the_map.get_at((int(check_x), int(check_y)))[:3]
                if pixel_color == target_color:
                    points = check_x * zoom,check_y * zoom
    ...
    def push(self,x,y,angle,rect,velocity, dt, power,collision_points):
        ...
        x += collision_direction.x *power*dt*4  
        y += collision_direction.y *power*dt*4
